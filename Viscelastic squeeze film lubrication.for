...
C
        PROGRAM PEHL2
        PARAMETER(M=3,NA=256,NB=256,NC=20,KD=1,PI=3.1415926,NT1=100,
     &            NT2=100)
     ...
     &            DB(M,0:NA,0:NB),DBS(0:NA,0:NB),PL(M,0:NA,0:NB,0:NT2)
	  DIMENSION XX(0:NT2),YY(0:NT2),HMIN(0:NT2),HMINY(0:NT2),
     &            PMAX(0:NT2),TIMES(0:NT2),H0T(0:NT2),DEFOR(0:NA,0:NB),
     &            EDD(M,0:NA,0:NB,0:NT2),ZS(0:NC),TNXX(0:NA,0:NB,0:NC),
     &            TNYY(0:NA,0:NB,0:NC),TNZZ(0:NA,0:NB,0:NC),
     &            TNXY(0:NA,0:NB,0:NC),TNXZ(0:NA,0:NB,0:NC),
     &            TNYZ(0:NA,0:NB,0:NC),
     &            TXX(0:NA,0:NB,0:NC),TYY(0:NA,0:NB,0:NC),
     &            TZZ(0:NA,0:NB,0:NC),TXY(0:NA,0:NB,0:NC),
     &            TXZ(0:NA,0:NB,0:NC),TYZ(0:NA,0:NB,0:NC),
     &            SIGMAV(0:NA,0:NB,0:NC)
...
C          
	  !计算应力系数
	  CALL TN(NC,NA,NB,TNXX,TNYY,TNZZ,TNXY,TNXZ,TNYZ,
     &                AV0,PI,DXS,DYS,DCS,ZS)
        
        CALL COEFFI(DB,DX,DY,NX,NY,M,NA,NB,KD)
C
C下面为将各层的弹性变形系数由时域变换到复频域
        FFT_FORWARD=1
	  DO 80 K=1,M
	  DO 20 I=0,NX(K)
	  DO 15 J=0,NY(K)
...
C       无量纲时间间隔
	  ...
	  DO 110 I=0,NA
	  DO 100 J=0,NB
	  H(M,I,J)=(10.0E-5)/A/A*RX+0.5*X(M,I)**2+0.5*Y(M,J)**2
  100   CONTINUE
  110   CONTINUE
	  GOTO 210
	  ENDIF
...
C
  ...
  160   CONTINUE
     ...
     &       KD,ISW,PI,DT0,NT,PL,EDD,TP,NT1,NT2,UDB)
C
        ERRP=0.0
        SUMP=0.0
        DO 180 I=0,NX(M)
...
C
        WRITE(*,*) '*********** OK ***********'
        IF((ERRP.LT.1.0E-3).AND.(ABS(ERRW).LT.1.0E-3)) GOTO 210
  200   CONTINUE
C
C
  210   XX=0.0
...
C
	  ...
	  IF(NT.EQ.1.OR.NT.EQ.25.OR.NT.EQ.50.OR.NT.EQ.100)THEN
	  !输入压力分布
	  DO 1000 I=0,NX(M)
        DO 900 J=0,NY(M)
	  PS(I,J)=P(M,I,J)
 ...
 1000   CONTINUE
	  ...
	  CALL STRESS(NA,NB,NC,L,TNXX,NXS,NYS,TXX,PS,PI)
	  CALL STRESS(NA,NB,NC,L,TNYY,NXS,NYS,TYY,PS,PI)
	  CALL STRESS(NA,NB,NC,L,TNZZ,NXS,NYS,TZZ,PS,PI)
	  CALL STRESS(NA,NB,NC,L,TNXY,NXS,NYS,TXY,PS,PI)
	  CALL STRESS(NA,NB,NC,L,TNYZ,NXS,NYS,TYZ,PS,PI)
	  CALL STRESS(NA,NB,NC,L,TNXZ,NXS,NYS,TXZ,PS,PI)
        DO 1200 I=0,NX(M)
	  DO 1100 J=0,NY(M)
	  SIGMAV(I,J,L)=(((TXX(I,J,L)-TYY(I,J,L))**2+
     &                (TXX(I,J,L)-TZZ(I,J,L))**2+
     &         (TZZ(I,J,L)-TYY(I,J,L))**2+
     &         6.0*(TXY(I,J,L)**2+TXZ(I,J,L)**2+
     &         TYZ(I,J,L)**2)**2)**0.5)/SQRT(2.0)
 1100   CONTINUE
 1200   CONTINUE
	  print *,'NT=',NT,'L=',L
 1300   CONTINUE
...
c
     ...
     &  PHERTZ,DEFOR,TNXX,TNYY,TNZZ,TNXY,TNXZ,TNYZ,NC,TXX,TYY,TZZ,TXY,
     &  TYZ,TXZ,ZS,SIGMAV)
     ...
     &  TYZ,TXZ,ZS,SIGMAV)
	  ...
	  CALL RESULT3(X,Y,P,H,M,NA,NB,XIN,XOUT,YIN,YOUT,PH,RK,RX,A,ICY,
     &  PHERTZ,DEFOR,TNXX,TNYY,TNZZ,TNXY,TNXZ,TNYZ,NC,TXX,TYY,TZZ,TXY,
     &  TYZ,TXZ,ZS,SIGMAV)
	  ENDIF
        DO 405 K=1,M
...
C
	  ...
	  DO 610 K=0,NT2
	  WRITE(16,650) TIMES(K),HMIN(K)*A*A/RX*1.0E6,
     &                HMINY(K)*A*A/RX*1.0E6, !μm
     &                PMAX(K)*PHERTZ*1.0E-6,H0T(K)*A*A/RX*1.0E6     !MPa
C                   
  610   CONTINUE
...
C
	  ...
	  SUBROUTINE STRESS(NA,NB,NC,K,TN,NXS,NYS,TNS,PS,PI)
	  DIMENSION TN(0:NA,0:NB,0:NC),TNS(0:NA,0:NB,0:NC),PS(0:NA,0:NB)
...
C再对每一行作IFFT
  ...
  240   CONTINUE
	  DO 260 I=0,NXS
	  DO 250 J=0,NYS
	  TNS(I,J,K)=REAL(UDE(I,J)/ND/NE)
  250   CONTINUE
  260   CONTINUE
	  END
	  
	  SUBROUTINE TN(NC,NA,NB,TNXX,TNYY,TNZZ,TNXY,TNXZ,TNYZ,
     &                AV0,PI,DXS,DYS,DCS,ZS)
	  DIMENSION  TNXX(0:NA,0:NB,0:NC),R(0:NA,0:NB,0:NC),
     &             R1(0:NA,0:NB,0:NC),R2(0:NA,0:NB,0:NC),
     &             R3(0:NA,0:NB,0:NC),R4(0:NA,0:NB,0:NC),
     &             ZS(0:NC),TNYY(0:NA,0:NB,0:NC),TNZZ(0:NA,0:NB,0:NC),
     &             TNXY(0:NA,0:NB,0:NC),TNXZ(0:NA,0:NB,0:NC),
     &             TNYZ(0:NA,0:NB,0:NC)
	  DOUBLE PRECISION XSP,YSP,XSM,YSM,R1,R2,R3,R4,
     &                   TNXX1,TNXX2,TNXX3,TNXX4,
     &                   TNYY1,TNYY2,TNYY3,TNYY4,
     &                   TNZZ1,TNZZ2,TNZZ3,TNZZ4,
     &                   TNXY1,TNXY2,TNXY3,TNXY4,
     &                   TNXZ1,TNXZ2,TNXZ3,TNXZ4,
     &                   TNYZ1,TNYZ2,TNYZ3,TNYZ4
	  ...
	  R1(I,J,L)=SQRT(XSP**2+YSP**2+ZS(L)**2)
	  R2(I,J,L)=SQRT(XSM**2+YSM**2+ZS(L)**2)
	  R3(I,J,L)=SQRT(XSM**2+YSP**2+ZS(L)**2)
	  R4(I,J,L)=SQRT(XSP**2+YSM**2+ZS(L)**2)
C  
	  IF(L.EQ.0)THEN
	  IF((XSP*YSP).GT.0)THEN
	  TNXX1=-2*AV0*(0.5*PI)+2.0*(1-2*AV0)*
     &  ATAN(XSP/(R1(I,J,0)+YSP))
	  TNZZ1=-PI/2
	  ELSEIF((XSP*YSP).LT.0)THEN
	  TNXX1=-2*AV0*(-0.5*PI)+2.0*(1-2*AV0)*
     &  ATAN(XSP/(R1(I,J,0)+YSP))
	  TNZZ1=PI/2
	  ENDIF
C	  
	  IF((XSM*YSM).GT.0)THEN
	  TNXX2=-2*AV0*(0.5*PI)+2.0*(1-2*AV0)*
     &  ATAN(XSM/(R2(I,J,0)+YSM))
	  TNZZ2=-PI/2
	  ELSEIF((XSM*YSM).LT.0)THEN
	  TNXX2=-2*AV0*(-0.5*PI)+2.0*(1-2*AV0)*
     &  ATAN(XSM/(R2(I,J,0)+YSM))
	  TNZZ2=PI/2
	  ENDIF
c
	  IF((XSM*YSP).GT.0)THEN
	  TNXX3=-2*AV0*(0.5*PI)+2.0*(1-2*AV0)*
     &  ATAN(XSM/(R3(I,J,0)+YSP))
	  TNZZ3=-PI/2
	  ELSEIF((XSM*YSP).LT.0)THEN
	  TNXX3=-2*AV0*(-0.5*PI)+2.0*(1-2*AV0)*
     &  ATAN(XSM/(R3(I,J,0)+YSP))
	  TNZZ3=PI/2
	  ENDIF
c
	  IF((XSP*YSM).GT.0)THEN
	  TNXX4=-2*AV0*(0.5*PI)+2.0*(1-2*AV0)*
     &  ATAN(XSP/(R4(I,J,0)+YSM))
	  TNZZ4=-PI/2
	  ELSEIF((XSP*YSM).LT.0)THEN
	  TNXX4=-2*AV0*(-0.5*PI)+2.0*(1-2*AV0)*
     &  ATAN(XSP/(R4(I,J,0)+YSM))
	  TNZZ4=PI/2
	  ENDIF
	  ...
	  TNXX1=-2*AV0*ATAN(XSP*YSP/R1(I,J,L)/ZS(L))+
     &  2.0*(1-2*AV0)*ATAN(XSP/(R1(I,J,L)+YSP+ZS(L)))-
     &  XSP*ZS(L)/(R1(I,J,L)+YSP)
	  TNXX2=-2*AV0*ATAN((XSM*YSM)/R2(I,J,L)/ZS(L))+
     &  2.0*(1-2*AV0)*ATAN(XSM/(R2(I,J,L)+YSM+ZS(L)))-
     &  XSM*ZS(L)/(R2(I,J,L)+YSM)
	  TNXX3=-2*AV0*ATAN((XSM*YSP)/R3(I,J,L)/ZS(L))+
     &  2.0*(1-2*AV0)*ATAN(XSM/(R3(I,J,L)+YSP+ZS(L)))-
     &  XSM*ZS(L)/(R3(I,J,L)+YSP)
	  TNXX4=-2*AV0*ATAN((XSP*YSM)/R4(I,J,L)/ZS(L))+
     &  2.0*(1-2*AV0)*ATAN(XSP/(R4(I,J,L)+YSM+ZS(L)))-
     &  XSP*ZS(L)/(R4(I,J,L)+YSM)
	  TNZZ1=-ATAN(XSP*YSP/R1(I,J,L)/ZS(L))+
     &  XSP*ZS(L)/R1(I,J,L)/(R1(I,J,L)+YSP)+
     &  YSP*ZS(L)/R1(I,J,L)/(R1(I,J,L)+XSP)
	  TNZZ2=-ATAN(XSM*YSM/R2(I,J,L)/ZS(L))+
     &  XSM*ZS(L)/R2(I,J,L)/(R2(I,J,L)+YSM)+
     &  YSM*ZS(L)/R2(I,J,L)/(R2(I,J,L)+XSM)
	  TNZZ3=-ATAN(XSM*YSP/R3(I,J,L)/ZS(L))+
     &  XSM*ZS(L)/R3(I,J,L)/(R3(I,J,L)+YSP)+
     &  YSP*ZS(L)/R3(I,J,L)/(R3(I,J,L)+XSM)
	  TNZZ4=-ATAN(XSP*YSM/R4(I,J,L)/ZS(L))+
     &  XSP*ZS(L)/R4(I,J,L)/(R4(I,J,L)+YSM)+
     &  YSM*ZS(L)/R4(I,J,L)/(R4(I,J,L)+XSP)
	  
	  ENDIF
	  TNXY1=(2.0*AV0-1.0)*LOG10(R1(I,J,L)+ZS(L))-ZS(L)/R1(I,J,L)
	  TNXY2=(2.0*AV0-1.0)*LOG10(R2(I,J,L)+ZS(L))-ZS(L)/R2(I,J,L)
	  TNXY3=(2.0*AV0-1.0)*LOG10(R3(I,J,L)+ZS(L))-ZS(L)/R3(I,J,L)
	  TNXY4=(2.0*AV0-1.0)*LOG10(R4(I,J,L)+ZS(L))-ZS(L)/R4(I,J,L)
	  
	  TNXZ1=-ZS(L)**2/R1(I,J,L)/(R1(I,J,L)+YSP)
	  TNXZ2=-ZS(L)**2/R2(I,J,L)/(R2(I,J,L)+YSM)
	  TNXZ3=-ZS(L)**2/R3(I,J,L)/(R3(I,J,L)+YSP)
	  TNXZ4=-ZS(L)**2/R4(I,J,L)/(R4(I,J,L)+YSM)
	  
	  TNYZ1=-ZS(L)**2/R1(I,J,L)/(R1(I,J,L)+XSP)
	  TNYZ2=-ZS(L)**2/R2(I,J,L)/(R2(I,J,L)+XSM)
	  TNYZ3=-ZS(L)**2/R3(I,J,L)/(R3(I,J,L)+XSM)
	  TNYZ4=-ZS(L)**2/R4(I,J,L)/(R4(I,J,L)+XSP)
	  
	  TNXX(I,J,L)=(TNXX1+TNXX2-TNXX3-TNXX4)/(2.0*PI)
	  TNZZ(I,J,L)=(TNZZ1+TNZZ2-TNZZ3-TNZZ4)/(2.0*PI)
	  TNXY(I,J,L)=(TNXY1+TNXY2-TNXY3-TNXY4)/(2.0*PI)
	  TNXZ(I,J,L)=(TNXZ1+TNXZ2-TNXZ3-TNXZ4)/(2.0*PI)
	  TNYZ(I,J,L)=(TNYZ1+TNYZ2-TNYZ3-TNYZ4)/(2.0*PI)
   60   CONTINUE
   70   CONTINUE
  100   CONTINUE
        DO 130 L=0,NC
...
C
     ...
     &             DB,DBS,KD,ISW,PI,DT0,NT,PL,EDD,TP,NT1,NT2,UDB)
     ...
     &            PL(M,0:NA,0:NB,0:NT2),
     &            EDD(M,0:NA,0:NB,0:NT2)
	  COMPLEX(8)::UDB(M,0:2*NA,0:2*NB)
        DO 10 K=1,M
        MC(K)=0
   10   CONTINUE
   ...
   20   NXS=NX(K)
     ...
     &       NA,NB,KEY,DB,DBS,KD,ISW,PI,DT0,NT,PL,EDD,TP,NT1,NT2,UDB)
	  MC(K)=MC(K)+1
   ...
   40   CONTINUE
     ...
     &       KD,PI,DT0,NT,PL,EDD,TP,NT1,NT2,UDB)
        K=K-1
        IF(K.GT.1) GOTO 20
        KEY=0
        NXS=NX(K)
...
C
     ...
     &             OMIG1,OMIG2,M,NA,NB,KEY,DB,DBS,KD,ISW,PI,
     &             DT0,NT,PL,EDD,TP,NT1,NT2,UDB)
        DIMENSION P(M,0:NA,0:NB),H(M,0:NA,0:NB),X(M,0:NA),Y(M,0:NB),
     &            FP(M,0:NA,0:NB),FG(M),DX(M),DY(M),
   ...
   55   CONTINUE
        ...
        DYS=DY(K)
        DYS2=DYS**2
C        D0=DBT(K,0,0,NT-K)此处的NT-K应该为0，对压力求导数只剩第NT时刻的系数，第NT-1的压力已知为常数
C        D1=DBT(K,1,0,NT)
        D0=DB(K,0,0)
	  D1=DB(K,1,0)
C
...
C
     ...
     &       NYS,M,K,KD,PI,DB,NT,PL,EDD,TP,NT1,NT2,UDB)
        DO 70 I=0,NXS
        DO 60 J=0,NYS
        EPS(I,J)=HS(I,J)**3/AL
        RH1(I,J)=HS(I,J)
...
C
  ...
  150   CONTINUE
     ...
     &       A4,Z0,NA,NB,DBS,M,K,KD,PI,DB,DT0,NT,PL,EDD,
     &       TP,NT1,NT2,UDB)
        DO 170 I=0,NXS
        DO 160 J=0,NYS
        RES(K,I,J)=RESD(I,J)
...
C
     ...
     &             M,K,KD,PI,DB,DT0,NT,PL,EDD,TP,NT1,NT2,UDB)
     ...
     &            DB(M,0:NA,0:NB),
     &            PL(M,0:NA,0:NB,0:NT2),EDD(M,0:NA,0:NB,0:NT2)
	  COMPLEX(8)::UDB(M,0:2*NA,0:2*NB)
        CALL FILMTH(HS,PS,XS,YS,DBS,H00,RD,AL1,NA,NB,NXS,
     &       NYS,M,K,KD,PI,DB,NT,PL,EDD,TP,NT1,NT2,UDB)
        DXS2=DXS**2
...
C
     ...
     &       NYS1,M,K1,KD,PI,DB,NT,PL,EDD,TP,NT1,NT2,UDB)
C
        DO 80 I=0,NXS1
        DO 70 J=0,NYS1
        EPS(I,J)=HS(I,J)**3/AL
...
C
     ...
     &             NYS,M,ML,KD,PI,DB,NT,PL,EDD,TP,NT1,NT2,UDB)
...
C	  DO 500 K=1,NT
	  ...
	  DO 20 I=0,NXS
	  DO 10 J=0,NYS
	  UPS(I,J)=PL(ML,I,J,NT)-PL(ML,I,J,NT-1)
C
   10   CONTINUE
   20   CONTINUE
...
C再对每一行作IFFT
  ...
  240   CONTINUE
	  DO 260 I=0,NXS
	  DO 250 J=0,NYS
	  ED(I,J)=REAL(UDE(I,J)/ND/NE)
        EDD(ML,I,J,NT)=ED(I,J)
  250   CONTINUE
  260   CONTINUE
	  DO 500 K=1,NT
	  DO 300 I=0,NXS
	  DO 280 J=0,NYS
	  HS(I,J)=HS(I,J)+(1.0+(NT-K)*DT/TL)*EDD(ML,I,J,K) !这地方好像可以用卷积，再想下!
  280   CONTINUE
  300   CONTINUE
  500   CONTINUE
	  DO 700 I=0,NXS
...
C
     ...
     &             RK,RX,A,ICY,PHERTZ,DEFOR,TNXX,TNYY,TNZZ,TNXY,
     &             TNXZ,TNYZ,NC,TXX,TYY,TZZ,TXY,
     &             TYZ,TXZ,ZS,SIGMAV)
        DIMENSION X(M,0:NA),Y(M,0:NB),P(M,0:NA,0:NB),H(M,0:NA,0:NB),
     &            DEFOR(0:NA,0:NB),
     &            TNXX(0:NA,0:NB,0:NC),TNYY(0:NA,0:NB,0:NC),
     &            TNZZ(0:NA,0:NB,0:NC),TNXY(0:NA,0:NB,0:NC),
     &            TNXZ(0:NA,0:NB,0:NC),TNYZ(0:NA,0:NB,0:NC),
     &            KX(0:NA),ZS(0:NC),SIGMAV(0:NA,0:NB,0:NC),
     &            TXX(0:NA,0:NB,0:NC),TYY(0:NA,0:NB,0:NC),
     &            TZZ(0:NA,0:NB,0:NC),TXY(0:NA,0:NB,0:NC),
     &            TXZ(0:NA,0:NB,0:NC),TYZ(0:NA,0:NB,0:NC)
        ...
        OPEN(19,FILE='TXYZ0.DAT')
	  OPEN(20,FILE='SIGMAV0.DAT')
        MK=1
        DO 6 I=0,NA
        DO 5 J=0,NB
...
C
  ...
  140   FORMAT(1X,F15.7,E15.7,E15.7)
	  ...
	  DO 19 I=0,NC
	  WRITE(19,20) ZS(I),TXX(128,128,I),TYY(128,128,I),TZZ(128,128,I),
     &               0.5*abs(TZZ(128,128,I)-TXX(128,128,I))
   19   CONTINUE
   20   FORMAT(1X,E15.7,E15.7,E15.7,E15.7,E15.7,E15.7,E15.7)
        
	  WRITE(20,105) 0,(X(M,I),I=0,NA,1)
  105   FORMAT(I12,10000(1X,F12.5))
	  DO 22 L=0,NC,1
	  WRITE(20,125) ZS(L),(SIGMAV(I,128,L),I=0,NA,1)
   22   CONTINUE
  125   FORMAT(F12.5,10000(1X,E12.5))
        CLOSE(9)
        CLOSE(10)
        CLOSE(11)
        CLOSE(12)
...
C
     ...
     &            TXZ(0:NA,0:NB,0:NC),TYZ(0:NA,0:NB,0:NC)
        ...
        OPEN(19,FILE='TXYZ1.DAT')
	  OPEN(20,FILE='SIGMAV1.DAT')
        MK=1
        DO 6 I=0,NA
        DO 5 J=0,NB
        H(M,I,J)=H(M,I,J)*A*A/RX*1.0E6
...
C
     ...
     &            TXZ(0:NA,0:NB,0:NC),TYZ(0:NA,0:NB,0:NC)
	  ...
	  OPEN(10,FILE='P2.DAT')
        ...
        OPEN(19,FILE='TXYZ2.DAT')
	  OPEN(20,FILE='SIGMAV2.DAT')
        MK=1
        DO 6 I=0,NA
        DO 5 J=0,NB
        H(M,I,J)=H(M,I,J)*A*A/RX*1.0E6
...
C        
     ...
     &            TXZ(0:NA,0:NB,0:NC),TYZ(0:NA,0:NB,0:NC)
	  ...
	  OPEN(10,FILE='P3.DAT')
        ...
        OPEN(19,FILE='TXYZ3.DAT')
	  OPEN(20,FILE='SIGMAV3.DAT')
        MK=1
        DO 6 I=0,NA
        DO 5 J=0,NB
        H(M,I,J)=H(M,I,J)*A*A/RX*1.0E6
